...
class CityscapesDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        # 1. Tránh trường hợp ảnh nhãn là ảnh 3 kênh màu, ép về 1 chiều
        if mask.ndim == 3:
            mask = mask[:, :, 0]
            
        # 2. Xử lý lỗi an toàn và QUAN TRỌNG NHẤT: Bắt buộc phải MAPPING (Dịch nguyên code cũ lỗi mIoU 0.2)
        # File labelIds.png của Cityscapes chứa giá trị ID từ 0 -> 33. Và có 1 giá trị đặc biệt là 255 (License Plate / Ignore).
        # Nếu không ánh xạ thì Mạng Neural (chỉ xuất ra 0-18) sẽ so sánh với (0-33) => Luôn chệch => mIoU = 0.
        mask_safe = mask.astype(np.int64)
        
        # Bất kỳ Pixel nào gán ID = 255 (hoặc bừa bãi ngoài phạm vi 33) => Ép hết về 0 (Unlabeled)
        mask_safe[mask_safe >= len(self.label_map)] = 0
        
        # ÁNH XẠ (Mapping từ 33 class gốc về 19 class đào tạo TrainId)
        mask = self.label_map[mask_safe]
